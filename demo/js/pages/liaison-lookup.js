	...
	function renderPage() {
		...
		main.innerHTML =
			RM.Components.pageHeader(
				...
				'Limited read-only access — search for a caller to see whether Human Services has an active case and who to contact.'
			) +
			RM.Components.alert('info', 'Confidentiality policy: you see only the client name match and assigned case manager contact. No addresses, risk levels, care plans, or case notes are shown.') +
			'<div class="card liaison-lookup-card">' +
			'<h2>Look up a caller</h2>' +
			'<p class="liaison-lookup-hint">Search by client name or phone number from your referral.</p>' +
	...
	function renderResults(query) {
		...
		if (!rows.length) {
			el.innerHTML = RM.Components.emptyState(
				...
				'No open Human Services case matches this search. You may proceed with your program\'s intake process.'
			);
			return;
		}
