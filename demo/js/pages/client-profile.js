...
(function () {
	...
	var TABS = ['Referral', 'Intake', 'Risk Assessment', 'Care Plan', 'Services', 'Case Notes', 'Documents', 'Reassessment History'];
	...
	function renderProfile(client) {
		...
		main.innerHTML =
			...
			'<div><h1>' + RM.Components.escapeHtml(client.name) + '</h1>' +
			'<p class="profile-meta">DOB: ' + RM.Components.formatDate(client.dob) + ' · ' + RM.Components.escapeHtml(client.phone) + '</p>' +
			'<p class="profile-meta">' + RM.Components.escapeHtml(client.address) + '</p></div></div>' +
			...
			'<p class="profile-view-note">Read-only 360° view — use <a href="' + RM.Links.page('case-workspace', { clientId: client.id }) + '">Case Workspace</a> to update records.</p>' +
