...
(function () {
	...
	RM.Components = {
		...
		renderRatingsCompare: function (previousRatings, newRatings) {
			var self = this;
			var rows = RM.FormHelpers.DOMAINS.map(function (domain) {
				...
				return '<tr class="' + (changed ? 'compare-changed' : '') + '">' +
					'<td>' + self.escapeHtml(RM.FormHelpers.formatDomain(domain)) + '</td>' +
