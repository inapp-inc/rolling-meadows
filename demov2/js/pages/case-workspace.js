...
(function () {
	...
	function saveIntake(client) {
		...
		RM.Workflow.setStage(client.id, client.incompleteIntake ? 1 : 2);
		showAlert('success', 'Referral and intake saved.');
		state.activeTab = client.incompleteIntake ? 'intake' : 'assessment';
	...
	function renderComprehensiveAssessmentTab(panel, client, readOnly) {
		...
		panel.innerHTML =
			'<div class="card"><h2>Intake summary</h2>' +
			...
			'<p><strong>Living arrangement:</strong> ' + RM.Components.escapeHtml(intake ? intake.livingArrangement || '—' : '—') + '</p>' +
			'<p><strong>Medical history:</strong> ' + RM.Components.escapeHtml(intake ? intake.medicalHistory || '—' : '—') + '</p></div>' +
			'<form id="comprehensive-form" class="card"><h2>Comprehensive Assessment</h2>' +
			'<div class="form-group"><label for="comprehensive-notes">Assessment summary</label>' +
			'<textarea id="comprehensive-notes" rows="5"' + dis(readOnly) + '></textarea></div>' +
			(!readOnly ? '<div class="form-actions"><button type="submit" class="btn btn-primary">Save &amp; Continue to Risk Prioritization</button></div>' : '') +
			'</form>';
		...
		if (!readOnly) {
			document.getElementById('comprehensive-form').addEventListener('submit', function (e) {
				...
				RM.Workflow.setStage(client.id, 2);
				showAlert('success', 'Comprehensive assessment saved.');
	...
	function renderRiskTab(panel, client, readOnly) {
		...
		panel.innerHTML =
			...
			'<h2>Risk Identification &amp; Prioritization</h2>' +
			ratingsTable('risk', existing ? existing.ratings : {}, readOnly) +
		...
		if (!readOnly) {
			document.getElementById('risk-form').addEventListener('submit', function (e) {
				...
				RM.Workflow.setStage(client.id, 3);
				showAlert('success', 'Risk ratings saved.');
	...
	function renderCarePlanTab(panel, client, readOnly) {
		panel.innerHTML =
			(!readOnly ? '<form id="careplan-form" class="card"><h2>Care / Service Plan Development</h2>' +
			'<div class="form-row">' +
	...
	function renderServicesTab(panel, client, readOnly) {
		...
		panel.innerHTML =
			'<div class="card"><h2>Service Coordination</h2>' +
	...
	function renderFollowupTab(panel, client, readOnly) {
		...
		panel.innerHTML =
			...
			'<div class="card"><h2>Ongoing Monitoring &amp; Follow-Up</h2>' +
	...
	function renderRehistory(clientId) {
		...
		el.innerHTML = list.map(function (r) {
			return '<div class="card reassessment-history-card"><div class="note-meta">' +
				...
				RM.Components.renderRatingsCompare(r.previousRatings, r.newRatings) + '</div>';
		}).join('');
	}
	...
	function renderClosureTab(panel, client, readOnly) {
		...
		panel.innerHTML =
			...
			'<form id="closure-form" class="card"><h2>Case Resolution / Closure</h2>' +
			...
			(!readOnly ? '<button type="submit" class="btn btn-primary">Complete Case Resolution</button>' : '') +
			'</form>';
