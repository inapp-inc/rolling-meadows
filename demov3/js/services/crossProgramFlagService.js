...
(function () {
	...
	function programLabel(programId) {
		return PROGRAM_LABELS[programId] || programId || 'Community Programs';
	}
	...
	RM.CrossProgramFlagService = {
		...
		formatMessage: function (flag) {
			...
			var phone = flag.caseManagerPhone && flag.caseManagerPhone !== '—'
				? ' at ' + flag.caseManagerPhone
				: '';
			return 'Active case in ' + flag.programLabel + ' — contact ' +
				flag.caseManagerName + phone + '. Case details are not shown per confidentiality policy.';
		},
			...
			matches.forEach(function (match) {
				...
				if (isActiveCase(client)) {
					...
					activeCases.push({
						...
						programLabel: programLabel(client.programId),
						score: match.score
					});
				}
			});
