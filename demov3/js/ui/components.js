...
(function () {
	...
	RM.Components = {
		...
		openSideDrawer: function (title, bodyHtml, onClose, options) {
			...
			var sideClass = options.side === 'left' ? ' side-drawer-left' : '';
			var overlay = document.createElement('div');
			overlay.className = 'drawer-overlay';
			overlay.setAttribute('role', 'presentation');
			overlay.innerHTML =
				'<aside class="side-drawer' + sideClass + '" role="dialog" aria-modal="true" aria-labelledby="drawer-title">' +
		...
		showDedupDrawer: function (matches, options) {
			options = options || {};
			if (!matches.length) {
				if (this._activeDrawer && this._activeDrawer.kind === 'dedup') {
					this.closeSideDrawer();
				}
				return;
			}
