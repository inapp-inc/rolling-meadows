...
(function () {
	...
	function renderForm() {
		...
		var categoryBanner =
			...
			' · <strong>Workflow:</strong> ' + RM.Components.escapeHtml(workflow.name) +
			' · <a href="' + RM.Links.page('case-creation') + '">Change</a></div>';

		main.innerHTML =
			RM.Components.pageHeader(ctx.title, { moduleId: 'cases', lead: 'Case creation — step 2 of 2' }) +
			categoryBanner +
			'<div id="alerts"></div>' +
			...
			'<div class="form-actions"><button type="submit" class="btn btn-primary">Save &amp; Open ' +
			RM.Components.escapeHtml(assessmentCtx.title) + '</button></div>' +
			'</form>';
	...
	function liveDedup() {
		...
		RM.Components.showDedupDrawer(matches, {
			onOpen: function (clientId) {
				window.location.href = FH.workspaceUrl(clientId, 'intake');
			}
		});
	}
