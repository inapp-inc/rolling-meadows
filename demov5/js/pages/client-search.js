	...
	function renderResults(clients, canDetail, user, query) {
		...
		var rows = clients.map(function (c) {
			...
			return '<tr class="client-search-row" data-client-id="' + RM.Components.escapeHtml(c.id) + '" role="button" tabindex="0" aria-label="' +
				RM.Components.escapeHtml(t('pages.clientSearch.viewCaseAria', { name: c.name })) + '">' +
				...
				'<td>' + RM.Components.escapeHtml(RM.I18n.clientStatusLabel(c.status)) + '</td></tr>';
		}).join('');
		...
		el.innerHTML = RM.Components.tableResponsive(
			...
			'<th>' + RM.Components.escapeHtml(t('pages.clientSearch.tableCategory')) + '</th>' +
			'<th>' + RM.Components.escapeHtml(t('pages.clientSearch.tableProcessStage')) + '</th>' +
			...
			'<th>' + RM.Components.escapeHtml(t('pages.clientSearch.tableStatus')) + '</th></tr></thead><tbody>' + rows + '</tbody></table>'
		);
